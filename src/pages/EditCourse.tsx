...
import { Save, BookOpen, CheckCircle2, MapPin, Trash2, Award, AlertCircle, Users, User, ChevronDown, Clock, Calendar, Copy } from 'lucide-react';
...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [courseRes, locRes, unreadRes, userRes] = await Promise.all([
          api.get(`/courses/${id}`),
          api.get('/locations'),
        ...
        if (courseRes.data.success) {
          ...
          setFormData({
            ...
            enrolled_locations: c.enrolled_locations?.map((l: any) => l._id || l) || [],
            enrolled_students: c.enrolled_students?.map((s: any) => s._id || s) || []
          });
        }
        ...
        showToast('ไม่สามารถโหลดข้อมูลรายวิชาได้', 'error');
  ...
  const handleCopyCourse = () => {
    ...
    const semesterParts = formData.semester.split('/');
    if (semesterParts.length === 2) {
      const term = parseInt(semesterParts[0]);
      const year = semesterParts[1];
      if (!isNaN(term)) {
        nextSemester = `${term + 1}/${year}`;
      } else {
        nextSemester = `${formData.semester}/Copy`;
      }
    } else {
      nextSemester = `${formData.semester}/Copy`;
    }
    ...
    setFormData({
      ...formData,
      course_name: `${formData.course_name} (คัดลอก)`,
    ...
    setIsCopyMode(true);
    showToast('เตรียมข้อมูลคัดลอกเรียบร้อย กรุณาตรวจสอบข้อมูลและกดบันทึกเพื่อสร้างวิชาใหม่', 'info');
  };

  const handleLocationToggle = (locId: string) => {
    setFormData(prev => {
      const exists = prev.enrolled_locations.includes(locId);
      return {
        ...prev,
        enrolled_locations: exists 
          ? prev.enrolled_locations.filter(id => id !== locId)
          : [...prev.enrolled_locations, locId]
      };
    });
  };
  ...
  const handleStudentToggle = (sId: string) => {
    setFormData(prev => {
      const exists = prev.enrolled_students.includes(sId);
      return {
        ...prev,
        enrolled_students: exists 
          ? prev.enrolled_students.filter(id => id !== sId)
          : [...prev.enrolled_students, sId]
      };
    });
  };
    ...
    try {
      if (isCopyMode) {
        ...
        if (response.data.success) {
          showToast('คัดลอกและสร้างรายวิชาใหม่สำเร็จ', 'success');
          navigate('/admin/courses');
        }
      } else {
  ...
  const handleDelete = async () => {
    ...
    const isConfirmed = await confirm({
      title: 'ลบรายวิชา',
      message: 'คุณต้องการลบรายวิชานี้ใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้',
  ...
  const isCopied = formData.course_name.includes('(คัดลอก)') || isCopyMode;
  ...
  return (
    <>
        <DashboardHeader 
          ...
          showTitle={true}
          title={isCopyMode ? "คัดลอกรายวิชาใหม่" : "แก้ไขข้อมูลรายวิชา"}
          subtitle={isCopyMode ? "กำลังเตรียมข้อมูลสำหรับวิชาใหม่" : `จัดการข้อมูลของ: ${formData.course_name}`}
        />
        ...
        <div className="max-w-4xl mx-auto space-y-8 mt-6 pb-20">
          <div className="flex justify-between items-center">
            <button 
              ...
              type="button"
              disabled={isSaving || isCopyMode}
              className="bg-blue-50 text-blue-600 px-6 py-2.5 rounded-xl font-black flex items-center gap-2 transition active:scale-95 border border-blue-100 hover:bg-blue-100 disabled:opacity-50 text-[10px] uppercase tracking-widest shadow-sm"
            >
            ...
            >
              ...
              {isCopyMode ? 'ยกเลิกการคัดลอก' : (isDeleting ? 'กำลังลบ...' : 'ลบรายวิชานี้')}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            <div className="bg-white p-10 rounded-[48px] border-2 border-white shadow-xl space-y-8">
              <h3 className="text-xl font-black text-slate-800 flex items-center gap-3">
                ...
                ข้อมูลรายวิชา {isCopyMode && <span className="text-blue-500 text-sm ml-2">(โหมดคัดลอก)</span>}
              </h3>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      required
                      disabled={!isCopied}
                      className={`w-full pl-12 pr-6 py-4 border-2 rounded-2xl outline-none font-bold shadow-inner transition-all ${!isCopied ? 'bg-slate-100 border-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-50 border-slate-100 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:bg-white text-slate-700'}`} 
                    />
                  </div>
                </div>
                <div className="space-y-2">
            ...
            <div className="bg-white p-10 rounded-[48px] border-2 border-white shadow-xl space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div 
                  onClick={isCopied ? () => setFormData({...formData, evaluation_type: 'Pass/Fail'}) : undefined}
                  className={`p-6 rounded-[32px] border-4 transition-all ${!isCopied ? 'cursor-not-allowed' : 'cursor-pointer'} ${formData.evaluation_type === 'Pass/Fail' ? 'border-blue-500 bg-blue-50/50 shadow-lg' : `border-slate-50 bg-slate-50/10 ${!isCopied ? 'opacity-60' : 'hover:border-slate-100'}`}`}
                >
                ...
                <div 
                  onClick={isCopied ? () => setFormData({...formData, evaluation_type: '0-4'}) : undefined}
                  className={`p-6 rounded-[32px] border-4 transition-all ${!isCopied ? 'cursor-not-allowed' : 'cursor-pointer'} ${formData.evaluation_type === '0-4' ? 'border-blue-500 bg-blue-50/50 shadow-lg' : `border-slate-50 bg-slate-50/10 ${!isCopied ? 'opacity-60' : 'hover:border-slate-100'}`}`}
                >
              ...
              <div className="space-y-4">
                {availableYears.length > 0 ? (
                  availableYears.map(year => {
                    ...
                    const isAllYearSelected = yearStudents.length > 0 && yearSelectedCount === yearStudents.length;
                  ...
                  <div className="py-10 text-center bg-slate-50/50 rounded-[32px] border-2 border-dashed border-slate-100">
                    <AlertCircle className="mx-auto text-slate-300 mb-2" size={32} />
                    <p className="text-slate-400 font-bold text-sm">ไม่พบนักศึกษาในเทอม {formData.semester || '-'}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-10">
              ...
              >
                ...
                {isSaving ? 'กำลังบันทึก...' : (isCopyMode ? 'บันทึกรายวิชาใหม่' : 'บันทึกการเปลี่ยนแปลง')}
              </button>
            </div>
          </form>
        </div>
